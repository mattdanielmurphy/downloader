...
class Download {
	...
	torrent(magnetLink) {
		...
		// 	},
		// 	{
		// 		onSubmit: (prompt, response) => if (response === 'local' || response === 'ssh') location = response
		// 	}
		// ).then(() => {
		location === 'ssh' ? this.downloadViaSSH(magnetLink) : this.downloadLocally(magnetLink)
		// })
	}
